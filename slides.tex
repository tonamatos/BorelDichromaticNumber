...
\textbf{So far:} at both the $2/3$-color threshold and the countable/uncountable threshold, there is a clean dichotomy with a one-element basis.
...
This gives $\Pi^1_2 \subseteq \Sigma^1_2$, contradicting the \alert{strictness of the projective hierarchy}.
\end{proof}$\chi(G)=2$
\end{frame}
...
In particular, dicoloring is \alert{not a CSP}---acyclicity cannot be expressed as a homomorphism to a fixed finite template.
\end{frame}
...
The proof: lift the classical NP-completeness reduction to the Borel setting, using Thornton's coding framework.
\end{frame}
...
\begin{lemma}[Thornton's toolkit]
...
\textbf{Hardness:} We have a $\Delta^1_1$ reduction from Borel 2-colorable 3-uniform hypergraphs ($\Sigma^1_2$-complete by Thornton) to Borel 2-dicolorable digraphs. Hence the latter is $\Sigma^1_2$-hard.
